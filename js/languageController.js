...
const languageController = (() => {
  ...
  const applyLanguage = (lang) => {
    ...
    if (lyricTextarea)
      lyricTextarea.placeholder = data.lyric_textarea_placeholder;

    const splitLyricBtn = document.querySelector("#split-lyric-btn");
    if (splitLyricBtn) splitLyricBtn.textContent = data.split_lyric;

    const uploadLrcLabel = document.querySelector(
      "#lrc-upload-container .upload-label span",
    );
    ...
    noneTexts.forEach((el) => {
      if (
        el &&
        (el.textContent.trim() === "无" ||
          el.textContent.trim() === "無" ||
    ...
    if (prev5sBtn)
      prev5sBtn.innerHTML = `<i class="fas fa-backward-step"></i> ${data.back_2s}`;

    const next5sBtn = document.querySelector("#next-5s");
    if (next5sBtn)
      next5sBtn.innerHTML = `<i class="fas fa-forward-step"></i> ${data.forward_2s}`;

    const playPauseBtn = document.querySelector("#play-pause");
    if (playPauseBtn)
      playPauseBtn.innerHTML = `<i class="fas fa-play"></i> ${data.play_pause}`;
    ...
    const placeholderText = document.querySelector(
      ".lyric-preview .placeholder-text",
    );
    if (placeholderText) {
    ...
    const previewPlaceholder = document.querySelector(
      "#sync-lyric-display .placeholder-text",
    );
    ...
    if (previewPrev2s)
      previewPrev2s.innerHTML = `<i class="fas fa-backward-step"></i> ${data.back_2s}`;

    const previewPlayPause = document.querySelector("#preview-play-pause");
    if (previewPlayPause)
      previewPlayPause.innerHTML = `<i class="fas fa-play"></i> ${data.play_pause}`;

    const previewNext2s = document.querySelector("#preview-next-2s");
    if (previewNext2s)
      previewNext2s.innerHTML = `<i class="fas fa-forward-step"></i> ${data.forward_2s}`;

    const backToEdit = document.querySelector("#back-to-edit");
    if (backToEdit)
      backToEdit.innerHTML = `<i class="fas fa-arrow-left"></i> ${data.back_to_edit}`;
    ...
    updateMetaField(
      "meta-offset",
      data.meta_offset,
      data.meta_offset_placeholder,
    );
    updateMetaField(
      "meta-length",
      data.meta_length,
      data.meta_length_placeholder,
    );
    ...
    updateMetaField(
      "meta-composer",
      data.meta_composer,
      data.meta_composer_placeholder,
    );
    updateMetaField(
      "meta-lyricist",
      data.meta_lyricist,
      data.meta_lyricist_placeholder,
    );
    updateMetaField(
      "meta-translator",
      data.meta_translator,
      data.meta_translator_placeholder,
    );
    updateMetaField(
      "meta-language",
      data.meta_language,
    ...
    const subtitleUploadLabel = document.querySelector(
      "#subtitle-converter-container .upload-label span",
    );
    if (subtitleUploadLabel)
      subtitleUploadLabel.textContent = data.upload_subtitle;

    const subtitleModeH3 = document.querySelector(
      "#subtitle-converter-container .subtitle-mode-container h3",
    );
    if (subtitleModeH3) subtitleModeH3.textContent = data.subtitle_convert_mode;

    const subtitleModeLabels = document.querySelectorAll(
      "#subtitle-converter-container .subtitle-mode-label span",
    );
    if (subtitleModeLabels && subtitleModeLabels.length >= 2) {
      if (subtitleModeLabels[0])
        subtitleModeLabels[0].textContent = data.subtitle_text_only;
      if (subtitleModeLabels[1])
              ...
              if (options) {
                options.forEach((option) => {
                  option.classList.toggle(
                    "active",
                    option.dataset.lang === selectedLang,
                  );
                });
              }
            })
...
window.addEventListener("load", function () {
  setTimeout(() => {
    if (
      typeof languageController !== "undefined" &&
      typeof window.updateParsingMode === "function"
    ) {
